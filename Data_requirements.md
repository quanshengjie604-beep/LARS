...
| `cac_usd` | float/null | USD/customer | Record calculation method in evidence |
| `sales_cycle_days` | float/null | Days | Use average or median consistently |
| `churn_rate_annual` | float/null | Fraction/year | Specify logo churn or revenue churn in evidence |
| `dau` | integer/null | Users/day | Record measurement window |
...
| `sam_usd` | float/null | USD |
| `som_usd` | float/null | USD |
...
```json
{
  ...
  "missing_fields": ["traction.dau", "financials.cash_balance_usd"],
  "contradicted_fields": []
}
```
